...
sed '$d' $file_name | ruby -e "total = 0; ARGF.each do |line| splited = line.chomp.split(' '); STDERR.puts \"#{splited[1]}, #{total}, #{splited[0]}\"; total = total + splited[0].to_i; end; puts total;"
